<?php
    ...
    // Função para construir o conteúdo da tabela PDF de bolsa_reservas por período e setor
    function ConstruirTablePDFBolsa($pdf, $resultado_consulta) {
        ...
        $pdf->Cell(70, 6, iconv('utf-8', 'iso-8859-1', 'Paciente'), 1, 0, 'C', true);
        $pdf->Cell(20, 6, iconv('utf-8', 'iso-8859-1', 'Bolsa'), 1, 0, 'C', true);
        $pdf->Cell(20, 6, iconv('utf-8', 'iso-8859-1', 'Número SUS'), 1, 0, 'C', true);
        $pdf->Cell(35, 6, iconv('utf-8', 'iso-8859-1', 'Hemocomponente / Atributos'), 1, 1, 'C', true);
    ...
    $query_bolsa_reserva = "SELECT cb.num_bolsa, cb.num_sus, h.sigla, cb.data_transfusao, cb.horario_inicio, 
    ...
    DateTime::createFromFormat('Y-m-d', $data_fim) !== false) {
        ...
        $query_bolsa_reserva .= " WHERE cb.data_transfusao BETWEEN '$data_inicio' AND '$data_fim' AND reserva = 'sim'";
    ...
    if (!empty($id_setor)) {
        ...
        $query_bolsa_reserva .= " AND cb.id_livro_setor IN ($id_setor)";
    }
    ...
    if (!empty($bolsa)) {
        ...
        $query_bolsa_reserva   .= " AND cb.num_bolsa = '$row_numero_bolsa[num_bolsa]'";
        pg_free_result($resultado_numero_bolsa);
    }

    $query_bolsa_reserva .= " ORDER BY cb.data_transfusao, cb.horario_inicio, nome";

    // Execute a consulta
    $resultado_consulta  = conecta_query($conexao, $query_bolsa_reserva);
    ...
    if ($quantidade_consulta > 0) {
        ...
        $pdf->Cell(230,10,iconv('utf-8', 'iso-8859-1', 'RELATÓRIO DE BOLSAS RESERVAS HUM'),0,1,'C');
        ...
        if (!empty($id_setor)) {
            ...
            // Usar IN para suportar múltiplos setores (ex: PA_GERAL)
            $query_setor     = "SELECT nome_setor FROM sth_setores WHERE id_setor IN ($id_setor)";
            $resultado_setor = conecta_query($conexao, $query_setor);

            if ($resultado_setor && pg_num_rows($resultado_setor) > 0) {
                $nomes_setores = array();
                while ($row_setor = pg_fetch_assoc($resultado_setor)) {
                    $nomes_setores[] = $row_setor['nome_setor'];
                }
                $nome_setor = implode(", ", $nomes_setores);
                $pdf->Cell(280,10,iconv('utf-8', 'iso-8859-1', "Setor: $nome_setor"),0,1,"");
                
            } else {
                $pdf->Cell(280,10,iconv('utf-8', 'iso-8859-1', "Setor: Todos"),0,1,"");
            }
        ...
        $pdf->Output('pdf_bolsas_reservas.pdf', 'D');
